...
class CitiesViewModel @Inject constructor(
    ...
    inner class EventHandler : BaseEventHandler() {
        fun onEvent(event: OnOpen) = loadCity(event.data)
    ...
    private fun create(data: Pair<String, CityWithRelationship>) {
        (state.category as? Success)?.data?.let { saveCities(mapOf(it)) }
    ...
    ) = scope.launch { GetCitiesUseCase().collect { block(it) } }
    ...
    ) = scope.launch { GetCitiesUseCase(id).collect { block(it) } }


    private fun updateState(
        cities: ResState<Map<String, CityWithRelationship>> = state.cities,
        category: ResState<Pair<String, CityWithRelationship>> = state.category,
        snackbar: SnackbarData = state.snackbar,
    ) {
        _state.update { it.copy(category = category, cities = cities, snackbar = snackbar) }
    }
}
