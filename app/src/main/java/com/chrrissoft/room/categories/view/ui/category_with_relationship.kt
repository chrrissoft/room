    ...
    ResState(state = state) { res ->
        ...
        if (showOrders) {
            ...
            OrderListSheet(
                ...
                onSelect = { order ->
                    ...
                    else data.orders.plus(order.second.order)).let {
                        onStateChange(
                            res.first to data.copy(
                                orders = it
                            )
                        )
                    }
        ...
        if (showPromotions) {
            ...
            PromotionListSheet(
                state = promotions,
                onSelect = { promotion ->
                    (if (data.promotions.contains(promotion.second.promotion)) data.promotions.minus(
                        promotion.second.promotion
                    )
                    else data.promotions.plus(promotion.second.promotion)).let {
                        onStateChange(
                            res.first to data.copy(
                                promotions = it
                            )
        ...
        if (showSales) {
            ...
            SaleListSheet(
                ...
                onSelect = { sale ->
                    ...
                    else data.sales.plus(sale.second.sale)).let {
                        onStateChange(
                            res.first to data.copy(
                                sales = it
                            )
        ...
        Column(modifier) {
            ...
            RoomDivider()
            SelectableRoomTextField(value = data.orders.joinToString { ", " }, onClick = {})
            Spacer(modifier = Modifier.height(10.dp))
            SelectableRoomTextField(value = data.promotions.joinToString { ", " }, onClick = {})
            Spacer(modifier = Modifier.height(10.dp))
            SelectableRoomTextField(value = data.sales.joinToString { ", " }, onClick = {})
        }
    }
}
