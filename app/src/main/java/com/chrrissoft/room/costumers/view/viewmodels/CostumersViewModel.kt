...
class CostumersViewModel @Inject constructor(
    ...
    inner class EventHandler : BaseEventHandler() {
        fun onEvent(event: OnOpen) = loadCostumer(event.data)
    ...
    private fun create(data: Pair<String, CostumerWithRelationship>) {
        ...
        updateState(costumer = Success(data))
    }
    ...
    ) = scope.launch { GetCostumersUseCase().collect { block(it) } }
    ...
    ) = scope.launch { GetCostumersUseCase(id).collect { block(it) } }
    ...
    private fun updateState(
        ...
        _state.update { it.copy(costumer = costumer, costumers = costumers, snackbar = snackbar) }
    }
}
