...
class CountriesViewModel @Inject constructor(
    ...
    inner class EventHandler : BaseEventHandler() {
        fun onEvent(event: OnOpen) = loadCountry(event.data)
    ...
    private fun create(data: Pair<String, CountryWithRelationship>) {
        ...
        updateState(country = Success(data))
    }
    ...
    private fun deleteCountries(data: Map<String, Country>) {
        ...
        deleteCountries(data) { updateState() }
    }
    ...
    ) = scope.launch { GetCountriesUseCase().collect { block(it) } }
    ...
    ) = scope.launch { GetCountriesUseCase(id).collect { block(it) } }
    ...
    private fun updateState(
        ...
        _state.update { it.copy(country = country, countries = countries, snackbar = snackbar) }
    }
}
